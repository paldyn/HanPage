/**
 * Issue #3474 browser regression — HWP5 EncryptVersion 4 password open.
 *
...
async function dialogState(page) {
  return await page.evaluate(() => {
    ...
    const label = document.querySelector('label[for="hwp-password-input"]');
    return {
...
runTest('Issue #3474 HWP5 암호 문서 열기', async ({ page }) => {
  ...
  assert(initialDialog.shown, '암호 HWP5 선택 시 입력 대화상자가 표시된다');
  ...
  await page.waitForFunction(
    () => window.__wasm?.getSourceFormat?.() === 'hwp' && window.__wasm?.pageCount === 64,
    { timeout: 30_000 },
  );
  ...
  assert(finalState.pageCount === 64, '올바른 암호로 실제 HWP5 fixture를 연다');
