...
- Canvas rendering (WASM/Web) + 문서 단위 Canvas2D/CanvasKit 자동 선택
...
- Studio, 브라우저 확장/embed, VS Code 뷰어의 기본 요청은 `auto`입니다. bounded document preflight가 완전하고 적격이며 필요한 문서 폰트를 해당 표면에서 준비할 수 있는 경우만 CanvasKit으로 고정합니다. 그 외 문서와 초기화/리소스 준비/런타임 실패는 해당 문서 revision 전체를 Canvas2D로 고정합니다. Studio 계열의 `?renderer=canvas2d`와 `?renderer=canvaskit`은 명시적 강제 선택입니다.
