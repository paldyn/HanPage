...
- HML(HWPML 2.9/2.91) 가져오기: 본문·서식·표·사각형 글상자 열람, HWP/HWPX로 저장
- 페이지네이션 (다단 분할, 표 행 분할), 머리말/꼬리말/바탕쪽/각주
- SVG 내보내기 (CLI) + Canvas 렌더링 (WASM/Web)
- 웹 에디터 + hwpctl 호환 API (30 Actions, Field API)
- 1,100+ 테스트

> HML은 읽기 우선의 제한 지원입니다. 현재 실제 corpus로 확인된 HWPML 2.9/2.91 구조만
> 가져오며 그림·수식·내장/외부 리소스는 경고 후 제외됩니다. HML 원본 덮어쓰기는 지원하지
> 않고 HWP 또는 HWPX로 다른 이름으로 저장합니다.
