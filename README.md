...
#### v0.7.19 사이클 (2026-07-17)

> v0.7.18 후속 patch — 저장 지오메트리 신호(intra-para vpos 리셋/되감김) 존중의 렌더·편집
> 양 경로 정합, 표 페이지네이션 정밀도, 메모리·PDF 결정성, HML 지원, legacy /web 제거

**렌더링 정합 — 저장 신호 존중**
- 문단 중간 vpos 리셋의 쪽 경계 승격 — 다단 하단 잘림·내용 소실 해소 (#2320/#2311/#2319/#2322)
- rowspan 병합 셀 선언-잔여 관례 확정 (한글 행 괘선 실측, #2291), 표 밀집 과소분할 (#2287)
- 바탕쪽 z-order — 본문 텍스트 가림 해소 (#2318), HWP5 취소선 strike shape whitelist (#2258)

**편집기 · 성능**
- 편집 vpos 재계산의 저장 리셋·문단 간격 보존 — 가짜 페이지 변동 60건 해소 (#2299)
- 거대 셀 편집 cache coherence (#2214), undo/redo stale 선택 정리 (#2303)
- BinData 지연 로딩 RSS 244MB→49MB (#2263), export-pdf 바이트 재현성 (#2269)

**포맷 · 프론트**
- HML(HWPML) 열기 + 의미 보존 저장 (#1157), MessageChannel 임베드 transport v1 (#2186)
- legacy /web 개발 앱 제거 — Studio 단독 기준 명료화 (#2313)
