...
- 작성일: 2026-07-07
- 현행화: 2026-07-17, #2125 canonical 이전과 #2313 legacy 제거 반영
- 단계: current 공개 계약·금지 목록 guardrail
...
- `rhwp-studio` 웹/PWA 앱
- `assets/fonts` canonical 자산과 target별 배포 경로
...
| `@rhwp/editor` 의존성 | package 파일은 `index.js`, `index.d.ts`, `transport.js`이며 런타임 dependency 없음 | `npm/editor/package.json` | 런타임 dependency 추가와 React/Vue/Svelte 등 runtime UI framework 도입 금지 |
...
| `rhwp-studio` 폰트 | `rhwp-studio/public/fonts`는 `../../assets/fonts` symlink | `ls -l rhwp-studio/public/fonts` | canonical source와 runtime `fonts/...` URL을 구분하고 target build를 함께 검증 |
| legacy `/web` | #2125에서 font ownership을 분리한 뒤 #2313에서 tracked 개발 앱을 제거 | #2125, #2313, Git history | current app으로 복원·복제하지 않음. historical evidence의 과거 경로는 보존 |
...
| 폰트 라이선스 | `assets/fonts`에는 재배포 가능한 폰트만 포함. 한컴/MS 저작권 폰트는 Git 미포함 | `assets/fonts/FONTS.md`, `THIRD_PARTY_LICENSES.md`, `mydocs/tech/font_fallback_strategy.md` | 저작권 폰트 번들 금지. 새 폰트는 라이선스 문서와 함께 추가 |

## 3. current app과 font ownership 경계

Phase A와 legacy cleanup 이후 current 구조는 다음과 같다.

1. 지원 web app은 `rhwp-studio`다.
2. canonical WOFF2와 license authority는 `assets/fonts`다.
3. Studio는 `rhwp-studio/public/fonts -> ../../assets/fonts`를 사용한다.
4. Chrome/Firefox는 canonical 전체를 `dist/fonts`로 복사한다.
5. Safari는 Chrome dist를 상속한 뒤 전용 source와 manifest를 적용한다.
6. VS Code는 canonical source에서 승인된 11개 subset을 `dist/media/fonts`로 복사한다.
7. source directory와 runtime `fonts/...` URL은 서로 다른 계약이다.

Git history와 #2023·#2124·#2125 문서에 남은 `/web` 표현은 당시 구조를 설명하는 evidence다. current
source나 manual에 legacy 앱을 archive/stub 형태로 복원하지 않는다.

금지:

- legacy app source를 Studio 또는 별도 repository directory로 복제.
- font path 변경과 UI/engine 리팩터링을 같은 실행 PR에 혼합.
- font loader의 `fonts/*.woff2` 경로를 바꾸면서 확장/VS Code build copy를 누락.
- 저작권 보호 대상 한컴/MS 폰트 파일을 Git 또는 배포 bundle에 추가.
...
- webpack copy plugin이 `assets/fonts`에서 승인된 11개 폰트만 `dist/media/fonts`로 복사한다.
...
## 9. 런타임 UI framework 도입 금지

React/Vue/Svelte 등 runtime UI framework 도입은 v1.0 프론트 리팩터링의 확정 금지 항목이다. 이유는
`@rhwp/editor` 하나가 아니라 다음 전체 계약 때문이다.
...
- 제거된 legacy `/web` source를 current tree에 복원·복제.
...
- v1.0 이후 runtime UI framework 도입 필요성과 전체 target migration 비용.
...
Phase B 재진단에서는 legacy group을 제외한 최신 metrics와 이 문서의 guardrail을 기준으로 다음을
실측한다.
