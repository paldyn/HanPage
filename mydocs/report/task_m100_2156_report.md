...
문자폭 사다리 통제 프로브(#2150 줄높이 공식 역이용)로 **한글이 함초롬(HCR)
계열 문서의 비한글 문자(라틴·숫자·구두점·U+00B7)를 HCR hmtx 가 아닌
...
- `text_measurement.rs`: `HAANSOFT_BATANG_ASCII`(95) + `haansoft_latin_override`
  — `measure_char_width_embedded` 단일 관문 삽입 (네이티브 전 경로).
- 유닛 테스트 `issue_2156_hcr_latin_uses_haansoft_metrics` + 픽스처
...
1. WASM(JS canvas) 측정 경로 동일 대체.
2. 함초롬돋움 등 대체 규칙 범위(Haansoft Dotum 가능성) 확인.
3. 80168 계열 em 상쇄 잔여 축 분해 — #2150 공식 해금 조건.
