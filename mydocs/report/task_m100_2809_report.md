# Task #2809 처리 결과 보고서 — 나눔정렬 마지막 줄 공백 분배
...
- 기존 공백 개수, slack, 자간·dash 산식은 변경하지 않음.
...
- 144dpi visual sweep: rhwp `845×1191`, PDF `844×1190`, 후보 `0/1`.
  마지막 글자는 clip 안에서 온전히 표시됐다.

visual sweep의 전체 잉크 일치 보조값은 `12.97842%`다. Poppler가 한컴 전용 폰트를
대체 렌더한 영향이 커 이 수치 자체는 합격 판정으로 사용하지 않았다. 저장 LineSeg,
문자 좌표, 한컴 화면과 144dpi 판정본의 대상 라벨을 함께 최종 근거로 삼았다.

### 기존 golden 영향

`exam-kor-page5.svg`의 `<보 기>`에서 `기`와 `>`만 각각 `+9.6px` 이동했다.
...
개체 수가 유지됐고, 허용오차 ±2px 기준 회귀는 0건이다.

## 5. 검증

- `CARGO_INCREMENTAL=0 cargo test --lib` — `2511 passed; 0 failed; 7 ignored`.
...
PR 생성 후 검토 번호에 맞춰 144dpi compare/overlay/review 판정 PNG를
`mydocs/pr/assets/pr{번호}_...` 이름으로 편입하고, 커밋 SHA 고정 URL로 PR에 게시한다.
