...
- → 하한 보정을 `per-row 합산 == 0` 인 완전 증발 클래스로 국소화. 59043=41
  핀 복구, 교육부 381쪽 + 오버플로 해소(p26 ymax 3027→741) 유지

## 잔여 (후속 추적)

- 범교과 연결맵 −34: 후속 섹션들의 동종 거대 표에서 −1~−4 씩 잔존. 주된 성분은
  ① 거대 셀 내부 stored vpos-reset(hard break)에서 컷이 조기 정지해 생기는
  sliver 조각(p26 = 17px 밴드 1유닛 — 한글은 만충), ② 선언>콘텐츠 슬랙의
  잔존 (#2237 소관). 47×9 는 +2 과보정 (sliver 2쪽).
