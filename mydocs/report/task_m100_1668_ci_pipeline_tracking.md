...
| 3 | #1849 | `devel` push profile 배치 재조정 | #1666 측정 결과를 근거로 `devel` push를 `release-test` 전체 회귀 + 제한적 `release` smoke로 조정할지 검토 |
...
2. #1849에서는 `devel` push를 `release-test` 전체 회귀 + 제한적 `release` smoke로 조정할지 검토한다.
3. #1667에서는 exact cache hit 이후에도 `Dirty rhwp` / `Compiling rhwp`가 남는 원인을 Build & Test cargo cache와
   CodeQL Rust analyze cache 범위를 분리해 판단한다.
4. #1665에서는 #1849 이후에도 남는 `devel` push wall time과 runner-minutes를 병렬화 효과 산정의 주요 입력으로 사용한다.
