...
- **핸들 없는 파일 제외**: 드롭·`input[type=file]`로 연 파일은 핸들이 없어 재열기 불가 → 목록에 기록하지 않음.
...
| `src/main.ts` | `loadBytes()`에 기록 훅(핸들 있을 때만) + `renderRecentSubmenu()` 동적 렌더 + MenuBar `onMenuOpen` 연결 |
...
- 핸들 없는 열기 경로(드롭/input/URL·복구)는 목록에 남지 않음(재열기 불가하므로 의도적).
- 최초 메뉴 open 시 async 렌더로 항목이 수십 ms 뒤 나타날 수 있음(hover 이동 시간 내 완료, 실사용 무영향).
- 향후: 개별 항목 제거(우클릭/x 버튼), 목록 개수 설정화 등은 별도 이슈 후보.

## 6-1. 리뷰 반영 (PR #2286, 2026-07-16)

중간에 "핸들 없는 열기 미등록"을 결함으로 오인해 바이트 스냅샷 저장으로 확장했다가,
리뷰(jangster77)에서 이슈 범위·보존 정책 이탈로 반려되어 **핸들-only 로 원복**했다
(경위: `troubleshootings/task_m100_2285_recent-disabled.md` 2차 증상). 본 보고서의
설계(핸들+메타 저장, 핸들 없는 경로 제외)가 최종 구현과 일치한다. 추가로:

- 재열기 UX 확정: 권한 거부 → 항목 유지+안내 / 파일 이동·삭제 → 항목 제거+안내
  (`src/recent/recent-open.ts` 로 규칙 분리).
- 회귀 테스트: `tests/recent-store.test.ts`(핸들-only 기록 제외, isSameEntry 동명
  공존, 동일 핸들 최신화, 상한 8, remove/clear), `tests/recent-open.test.ts`(UX 3규칙).
