...
> **[2026-07-25 실험 종료]** 작업지시자 결정으로 self-hosted 러너 운영을 종료하고 검증
> 워크플로 전체를 GitHub 호스티드로 원상복원했다(#3284 이전 + timeout-minutes 유지).
> 결정 배경: 공유 홈 레이스 계열 결함이 연쇄 발생(아래 점검표 — 전부 정정은 했으나
> 운영 복잡도가 큼), CPU 공유로 시간 상한 테스트 flake, GitHub Pro 전환으로 호스티드
> 동시 job 20→40 상향. 이 문서는 재실험 시의 함정 카탈로그로 존치한다.
> 유지된 개선: timeout-minutes, `rhwp_bin()` 테스트 규약, install-wasm-pack 원자 설치.
