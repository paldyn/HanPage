...
`edit.ts:153` 의 "대화상자 없이 찾기 다음"(F3) 경로는 본문 좌표만 다루므로 `includeCells` 를
켜지 않았다 — 켜면 셀 매치를 받고도 이동하지 못한다. 대화상자와 동작이 갈리므로 후속으로
`navigateToHit` 를 공용 헬퍼로 빼서 양쪽이 같이 쓰게 하는 편이 좋다.
