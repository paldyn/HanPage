...
그대로 쓰지 않는다. CLI 의 `--timeout-seconds` 를 900~1800초처럼 충분히 크게 지정한다. 이때
`hwp-convert-mcp-client-20260713-075145.tar.gz` 또는 그 이후 client를 사용한다. 이 client는 변환 옵션과
동일한 timeout(추가 120초 여유)을 MCP SDK 요청에도 적용한다. VS Code MCP
