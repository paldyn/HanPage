...
### 9.3.1 mydocs 문서 전용 후속 커밋 fast-pass

collaborator 가 외부 PR head 에 review/운영 문서처럼 `mydocs/**` 아래 문서만 추가한 경우, 다음 조건을 모두
만족하면 heavy CI 가 job-level 로 skip 될 수 있다.

- PR head 의 뒤쪽 후속 커밋들이 `mydocs/**` 만 변경한다.
...
- `mydocs/**` 밖의 파일 변경 또는 문서가 아닌 실행/검증 입력 파일 변경
...
- PR head 최신 커밋 기준 GitHub Actions 통과 또는 `mydocs/**` 문서 전용 후속 커밋 fast-pass 결과 확인
