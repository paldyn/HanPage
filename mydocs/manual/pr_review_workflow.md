...
초기에 만든 항목은 쉽게 충돌하거나 오래된 상태가 된다. 따라서 PR 번호가 발급되고 변경 범위·검증 결과·merge
판단이 확정된 **최종 PR 준비 시점**에 최신 `devel`의 오늘할일을 먼저 반영한 뒤, 최종 PR review 문서 묶음과
같은 커밋으로 오늘할일을 생성·갱신한다.

PR 생성 전에는 오늘할일 항목을 만들지 않는다. PR 번호를 얻기 위해 최초 PR 을 연 뒤 review 문서·asset·오늘할일
후속 커밋을 준비하는 경우에도, push 직전 최신 `upstream/devel` 기준 파일과 충돌 여부를 다시 확인한다.
