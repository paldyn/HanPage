...
visual sweep 을 실제 리뷰 근거로 사용했다면, 리뷰 완료 전 선택한 대표 `review_NNN.png` 를 반드시
`mydocs/pr/assets/` 아래의 PR 번호가 포함된 안정 파일명으로 복사한다. PR review 문서에는 임시 산출물
경로와 최종 asset 경로를 둘 다 기록한다. 원시 PR comment 에서 이미지를 보여줄 예정이면 `output/...`
임시 경로만 남기고 끝내지 않는다.
...
문서-only PR 또는 후속 기록 fast-pass PR 도 후속 코멘트 예외가 아니다. heavy job 이 `skipped` 로 끝난 경우에도
PR 에 다음 내용을 남긴다.
...
fast-pass 는 CI 실행 범위 최적화일 뿐 후속 처리 생략 조건이 아니다. 문서-only / review-only fast-pass PR 도
merge 후 7.1 이슈 close 확인, 7.2 PR 코멘트, 7.3 후속 문서 처리 여부 확정을 동일하게 수행한다.
