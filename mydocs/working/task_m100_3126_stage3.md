...
- 클릭 직후 `PDF 준비 중… (0/N)` 토스트와 상태바 표시
- 완료 안내를 읽을 수 있는 최소 시간 보장 후 native print UI 호출
- native print UI 호출 전에 안내 토스트 제거(브라우저 모달 뒤 가림 방지)
- print 호출 시 상태바 `PDF 준비 완료`
- file handle·파일명·dirty 상태가 전/호출/후 모두 동일
- 호출 반환 뒤 iframe 제거
- searchable SVG `<text>`와 body text 보존

### 3.1 Chrome 수동 피드백 반영

1차 Chrome 수동 검증에서 다음 두 UX 문제가 확인됐다.

1. 상태바에는 `PDF 준비 중… (10/10)`이 있었지만 native print UI가 즉시 화면을 덮어 클릭
   피드백으로 인지하기 어려웠다.
2. 우상단 안내 토스트는 웹 페이지 요소이므로 브라우저 native print UI보다 위에 표시할 수 없고,
   print UI 바깥쪽에 일부만 남아 문장이 가려졌다.

따라서 z-index를 올리는 대신 준비 피드백의 수명주기를 바꿨다. PDF 명령은 0/N부터 토스트와
상태바를 갱신하고, 준비 완료 시 남은 브라우저 단계를 안내한 뒤 토스트를 즉시 제거하고
`print()`를 호출한다. 짧은 문서에서도 전체 피드백은 최소 1.2초, 완료 상태는 최소 0.3초
노출한다. 일반 인쇄는 PDF 안내 없이 같은 print surface를 사용한다.

기존 별도 창은 복원하지 않는다. #3126 결정에서 보존할 것은 `인쇄`라는 물리 프린터 진입점의
의미이며, `about:blank` 별도 창과 rhwp 내부 인쇄 버튼 재클릭은 안정화 대상인 구현 세부다.
별도 미리보기 수요가 확인되면 이후 `인쇄 미리보기…` 기능으로 분리한다.
