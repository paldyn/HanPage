...
- in-flight set은 동일 Service Worker 안의 동시 처리만 담당한다. Service Worker 재시작 경계는 기존
...
- 작업지시자가 수행한 1~7 정상 수명주기 테스트는 보강 전 빌드 기준이다. 보강 후에는 자동 열기를
  켠 상태에서 과거 HWP 다운로드 기록을 남기고 확장 Reload/Chrome 재실행 시 탭 0개, 새 HWP
  다운로드 시 탭 정확히 1개인지만 추가 확인한다.

## 판정

과거 다운로드 기록 전체를 다시 여는 #1498 경로는 유지된 가드와 회귀 테스트로 차단되어 있다.
감사에서 발견한 동일 id 동시 처리와 terminal state 드리프트도 재현 테스트와 함께 보강했다.
PR은 “재현되지 않은 제보에 대한 방어 로직과 관측 가능한 오류 처리”로 설명하되, 위 수동 인수
두 항목을 PR 전 확인한다.
