...
최신 배포 MCP client로 평문·암호 HWPX를 각각 변환해 PDF 1.7/A4/24쪽, server validation `ok`를
확인했다. 잘못된 암호 입력은 PDF를 만들지 않고 실패했다. 이 확인은 비밀번호·token을 노출하지 않는
stdin 입력으로 수행했다. 이 결과는 기준 PDF를 생성하는 MCP 경로의 정상성만 의미하며, rhwp renderer
정합 판정에는 사용하지 않는다.
...
| HWPX 비교 문서 ↔ 새 MCP 한컴 PDF | 양쪽 24쪽, 1쪽 `visual_accuracy_proxy_percent` 20.22520 | 동일 PUA 머리말이 표준 한글로 이어지고 Enter tofu는 의미 fallback으로 바뀜. 실제 키캡 조판·본문 및 그림 차이는 남음 |
...
- HWPX compare: `tmp/pdfs/task3486/sweep-hwp5-pua-v5.N1hM3r/hwp5-pua-display-v5/compare/compare_001.png`
- HWPX overlay: `tmp/pdfs/task3486/sweep-hwp5-pua-v5.N1hM3r/hwp5-pua-display-v5/overlay/overlay_001.png`
- HWPX review: `tmp/pdfs/task3486/sweep-hwp5-pua-v5.N1hM3r/hwp5-pua-display-v5/review/review_001.png`

레거시 한컴 PDF의 PUA 텍스트층 때문에 `pdftotext -bbox-layout`이 종료한 경우가 있었으나, PDF
raster와 SVG raster는 정상 생성됐다. 따라서 이번 단계에서는 marker 분석만 생략했고 24쪽 raster
overlay를 계속 사용했다.
