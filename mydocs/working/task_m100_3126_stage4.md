...
- 작성일: 2026-07-23
- 상태: **후속 UX 구현 및 Chrome 자동·사용자 수동 검증 완료, Windows Edge 검증 대기**
...
사용자가 PDF를 인쇄의 변형보다 저장 결과 형식으로 먼저 인식하도록 `PDF로 저장…`을
`다른 이름으로 저장(A)...` 바로 아래로 이동했다. 명칭, 아이콘, 단축키와 실행 동작은 변경하지 않았다.

파일 메뉴의 저장 영역은 `저장 → 다른 이름으로 저장 → PDF로 저장 → HWP 형식으로 저장 →
HWPX 형식으로 저장` 순서이며, 이후 구분선 아래에 `편집 용지 → 인쇄`가 이어진다.
계약 테스트에서 이 상대 순서를 고정한다.
