...
- 기존 `scripts/visual_sweep.py`는 SVG/PDF PNG와 좌우 비교 이미지를 생성하지만, 실제 PDF와 rhwp 결과의 차이를 수치화하지 못한다.
...
- `scripts/visual_sweep.py`에서 render tree JSON을 생성/분석한다.
...
- `scripts/visual_sweep.py`가 PDF/SVG PNG 비교와 render tree bbox 분석을 함께 수행하도록 확장했다.
...
- `scripts/visual_sweep.py`는 각 대상별로 다음 산출물을 추가 생성한다.
