# Task #2809 Stage 1 — Split 마지막 줄 공백 분배 정정
...
- `src/renderer/layout/paragraph_layout.rs`
  ...
  - 공백 개수, slack 계산, `Distribute` 글자 간격 산식은 변경하지 않았다.
...
- 수정 후: `416.2533 / 458.9067 / 501.5600px`.
...
- 두 경우 모두 첫 글자와 마지막 글자가 각 bbox 안에 들어가며, 144dpi 래스터에서
  마지막 글자의 잘림이 없음을 확인했다.
...
- `--dpi 144` 실측:
  ...
  - 기준 PDF PNG `844×1190`
  - 자동 후보 `flagged=0/1`
  - `pixel_match=89.31602%`, `visual_accuracy_proxy=12.97842%`
...
`exam-kor-page5.svg`의 `<보 기>`는 `기`와 `>`가 각각 `+9.6px` 이동했다.
