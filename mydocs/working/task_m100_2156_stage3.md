...
- `haansoft_latin_override(primary_name, c)` — 함초롬/HCR 접두 폰트의
  0x21..0x7F + U+00B7 오버라이드. **공백(0x20) 제외** (useFontSpace=0 고정
  0.5em 기존 경로 유지). 한글 음절 등은 기존 HCR hmtx.
- `measure_char_width_embedded` 최상단 삽입 — 모든 네이티브 측정 경로의
  단일 관문 (WASM JS-canvas 경로는 범위 외, 후속).
...
- 소스: `text_measurement.rs` (테이블+오버라이드+유닛 테스트
  `issue_2156_hcr_latin_uses_haansoft_metrics`)
...
1. WASM 측정 경로(JS canvas) 동일 대체.
2. 함초롬돋움 등 타 한컴 폰트 대체 규칙 범위 확인 (현 구현은 함초롬*/HCR*
   접두 전체에 Haansoft Batang 적용 — 돋움 계열은 Haansoft Dotum 가능성).
