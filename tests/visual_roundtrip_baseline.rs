...
const VISUAL_XFAIL: &[(&str, &str)] = &[
    ...
    // opengov 실문서 말뭉치(Task #1564) 신규 편입분 중 HWPX 라운드트립 구조 불일치 3건.
    // 본질 정정(직렬화 노드 삽입/삭제)은 별도 이슈 — 회귀 위험 분리. 나머지 6건은 PASS.
    (
        "opengov/36385464_결재문서본문_근무변경(반포수난구조대, 2026-06).hwpx",
        "구조 불일치 1페이지(노드 삽입/삭제)",
    ),
    (
        "opengov/36387103_결재문서본문_수질검사 신청 민원 처리 결과 안내.hwpx",
        "구조 불일치 1페이지(노드 삽입/삭제)",
    ),
    (
        "opengov/36388571_결재문서본문_2026년 6월 생일 축하품 수령증 제출(119항공대).hwpx",
        "구조 불일치 1페이지(노드 삽입/삭제)",
    ),
];
